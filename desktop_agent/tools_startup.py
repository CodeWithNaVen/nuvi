"""
Windows auto-start management for NUVI (V2).

Manages a single registry entry under
    HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Run\\Nuvi
which points at the silent launcher batch file (start-nuvi-silent.bat) located
...
VALUE_NAME = "Nuvi"
SILENT_LAUNCHER = "start-nuvi-silent.bat"
...
def _ensure_launcher_exists() -> str:
    """
    Make sure start-nuvi-silent.bat exists in the project root.
...
def enable_auto_start(args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        "result": "Auto-start enabled. Nuvi will launch silently on next Windows login.",
        "enabled": True,
        "launcher": launcher,
...
def disable_auto_start(args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        "result": "Auto-start disabled. Nuvi will no longer launch on login.",
        "enabled": False,
    }
...
def get_auto_start_status(args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        "result": (
            "Auto-start is ENABLED. Nuvi launches on Windows login."
            if enabled
