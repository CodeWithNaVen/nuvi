# NUVI Desktop Control Agent

A local Python FastAPI service that gives NUVI **JARVIS-style desktop control** —
...
> **This agent does NOT modify NUVI's UI, personality, or chat system.** It is a pure
> backend tool layer that NUVI's existing Node bridge (`server.ts`) calls over HTTP.
...
The agent binds to `127.0.0.1:8765`. Then start NUVI normally with `npm run dev`.
...
| `saveScreenshot` | Save screenshot to Pictures/NuviScreenshots |
...
- **Power actions** (shutdown, restart, sleep, lock) require a **two-step confirmation token**: NUVI must first call `requestPowerAction` (which issues a single-use, 60-second token), ask the user out loud to confirm, then call `executePowerAction` with the token. Without a valid token, the action is refused.
...
```
NUVI voice chat (existing, untouched)
        ↓
