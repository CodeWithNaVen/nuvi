"""
NUVI Desktop Control Agent — FastAPI entrypoint.

Single dispatch endpoint POST /execute { tool, args } -> { result } | { error }.
NUVI's Node bridge (server.ts) calls this over HTTP on 127.0.0.1:8765.
...
log = logging.getLogger("nuvi.desktop")
...
async def lifespan(app: FastAPI):
    log.info("NUVI Desktop Control Agent v%s starting up.", __version__)
    yield
    ...
    log.info("NUVI Desktop Control Agent stopped.")


app = FastAPI(
    title="NUVI Desktop Control Agent",
    version=__version__,
    description="JARVIS-style desktop automation backend for NUVI.",
    lifespan=lifespan,
)
...
def health() -> Dict[str, Any]:
    return {
        "status": "ok",
        "name": "NUVI Desktop Control Agent",
        "version": __version__,
