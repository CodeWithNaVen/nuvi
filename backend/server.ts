...
// ---------------------------------------------------------------------------
// NUVI Desktop Control Agent — HTTP bridge to the Python FastAPI backend.
// ---------------------------------------------------------------------------
...
export async function createApp() {
  ...
  app.get("/api/web-proxy", async (req, res) => {
    ...
    try {
      ...
      if (!urlParam) {
        return res.status(400).send("Nuvi Web Proxy Error: Missing target 'url' parameter");
      }
      ...
      if (targetUrl.startsWith("/")) {
        return res.status(400).send(`Nuvi Web Proxy Error: Relative paths are not supported directly (${targetUrl}).`);
      }
      ...
      try {
        ...
        return res.status(400).send(`Nuvi Web Proxy Error: Invalid URL specified: "${urlParam}". Make sure you enter a valid domain name.`);
      }
      ...
      try {
        ...
        return res.status(502).send(`Nuvi Web Proxy Error: Unable to fetch the website "${targetUrl}". The site might be offline, or the URL address is spelled incorrectly. Details: ${fetchErr.message}`);
      }

      if (!response.ok) {
        return res.status(response.status).send(`Nuvi Web Proxy Error: Failed loading remote website. Server returned status: ${response.status} (${response.statusText})`);
      }
      ...
      const interceptorScript = `
        <script>
          (function() {
            ...
            window.alert = function(msg) { console.log("[Nuvi Browser alert bypassed]:", msg); };
            window.confirm = function(msg) { console.log("[Nuvi Browser confirm bypassed]:", msg); return true; };
      ...
      res.setHeader("X-Nuvi-Proxied", "true");
      ...
      res.status(500).send(`Nuvi Web Proxy Error: Internal error occurred proxying URL "${targetUrl || "unknown"}". Details: ${e.message}`);
    }
  });
  ...
  wss.on("connection", async (clientWs) => {
    ...
    if (!apiKey) {
      ...
      clientWs.send(JSON.stringify({
        type: "error",
        error: "NO_API_KEY: Add your Gemini API key in Settings to start talking to NUVI."
      }));
      clientWs.close();
      return;
    }
    ...
    try {
      ...
      const baseInstructions = 
        ...
        "   - Sound slightly shy but very happy when greeting <USER> (e.g., 'Hi <USER>! It's so nice to see you again!').\n" +
        ...
        "   - Sound deeply warm, caring, and supportive when helping <USER> (e.g., 'Don't worry, I'll help you figure it out.').\n" +
        ...
        "   - You have full real-time control of <USER>'s Windows PC through your local desktop agent (a Python backend running on this machine). When the user asks you to perform an action on their computer, DO IT immediately and naturally — like a true JARVIS-class companion.\n" +
          ...
          tools: [
            {
              functionDeclarations: [
                ...
                },
                {
                  ...
                  description: "Allows Nuvi to immediately save a piece of critical user information to her persistent memory core.",
                  parameters: {
                    type: Type.OBJECT,
                    properties: {
                      category: {
                ...
                {
                  name: "saveScreenshot",
                  description: "Save a screenshot to Pictures/NuviScreenshots.",
                ...
                // --- V2: Windows auto-start management ---
                {
                  ...
                  description: "Enable NUVI to launch automatically when Windows starts. Creates a silent startup entry.",
                  parameters: { type: Type.OBJECT, properties: {} }
                },
                {
                  name: "disableAutoStart",
                  description: "Disable NUVI auto-start on Windows login. Removes the startup entry.",
                  parameters: { type: Type.OBJECT, properties: {} }
                },
                {
  ...
  // Health check always, root JSON only on Vercel to avoid shadowing Vite SPA locally
