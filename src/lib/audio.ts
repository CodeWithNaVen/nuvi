/**
 * Audio handling utility for Nuvi Live API Voice stream.
 * Handles:
...
export class NuviAudioSession {
  ...
  public async connect() {
    ...
    try {
      ...
      // Uses VITE_BACKEND_URL when deployed (https://nuvi.onrender.com), else same-origin
      this.ws = new WebSocket(wsUrl("/live"));
      this.ws.binaryType = "blob";

      this.ws.onopen = async () => {
        console.log("[Nuvi] Connected to server side WS bridge");
        try {
      ...
      this.ws.onmessage = async (event) => {
        try {
          ...
          if (data.type === "status") {
            console.log("[Nuvi WS Status]:", data.status);
          ...
          // Handle interruption signal (e.g. user talked over Nuvi)
          ...
          if (data.type === "turnComplete") {
            // Once Nuvi completes speaking, change visual state back to listening
            setTimeout(() => {
